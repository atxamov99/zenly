...
export default function AuthPage({
  ...
  const handleSubmit = async (event) => {
    ...
    setLoading(true)
    try {
      await onAuthenticate(form, mode === 'register' ? 'register' : 'login')
    } catch (err) {
        ...
        <div className={`w-full max-w-md rounded-[2rem] border p-5 backdrop-blur-xl ${isLight ? 'border-stone-200 bg-[#faf7f3]/94 shadow-[0_12px_28px_rgba(120,113,108,0.08)]' : 'border-white/15 bg-white/10 shadow-float'}`}>
          ...
          <form className="mt-5 space-y-4" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={loading}
              className={`inline-flex w-full items-center justify-center gap-2 rounded-2xl px-4 py-3 text-sm font-black uppercase tracking-[0.16em] shadow-lg disabled:opacity-60 ${
                isLight ? 'bg-stone-900 text-stone-50' : 'bg-white text-slate-950'
              }`}
            >
              <LogIn className="h-4 w-4" />
              {loading ? '...' : mode === 'register' ? t('continue') : t('signIn')}
            </button>
          </form>
        </div>
      </section>
